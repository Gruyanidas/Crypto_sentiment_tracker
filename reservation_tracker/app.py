...
def calendar_view(year=None, month=None):
    ...
    blocked_days = database.get_blocked_days_in_month(year, month)
...
def day_view(date_str):
    ...
    return render_template("day.html",
        ...
        slots=slots,
        is_blocked=database.is_day_blocked(date_str),
        year=day_date.year,
        month=day_date.month,
    )


# ── Block / unblock a whole day (non-working day) ───────────────────────────────

@app.route("/toggle_block/<date_str>", methods=["POST"])
@login_required
def toggle_block(date_str):
    try:
        ...
        return redirect(url_for("calendar_view"))
    now_blocked = database.toggle_day_blocked(date_str)
    if now_blocked:
        flash("Dan je označen kao neradni — klijentima se ne nudi na sajtu. 🔒", "success")
    else:
        flash("Dan je ponovo radni — termini su opet dostupni. 🔓", "success")

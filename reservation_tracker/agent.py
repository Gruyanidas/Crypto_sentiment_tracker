...
def _free_times_for(d, names, today, now_hour):
    ...
    # A day the owner marked as non-working has no free slots at all.
    if database.is_day_blocked(d.isoformat()):
        return []
...
def create_reservation(client_name, phone, service_type, date_str, time_str, lang="sr"):
    ...
    if database.is_day_blocked(d.isoformat()):
        return {"ok": False, "error": "day_closed"}
...
def _system_prompt(lang):
    ...
    common = (
        ...
        "- If get_free_slots returns note 'closed' for a date, the salon is NOT working that day. "
        "Tell the visitor warmly that the salon is closed that day (do not present it as merely fully "
        "booked) and kindly offer another day. Never offer a time or take a booking on a closed day.\n"
        ...
        "- You CAN book the appointment for the visitor yourself, right here in the chat — and this is "
        "the EASIEST way: they just say when they'd like to come and you handle the rest. Whenever "
        "booking comes up, proactively make this clear (e.g. 'mogu odmah da vas zakažem — samo recite "
        "koji termin vam odgovara'). The on-page booking form and the phone (063 304 700) are only "
        "alternatives for those who prefer them; NEVER imply the form is the only way to book. "
        "Booking has three steps, in strict order:\n"
        ...
        "with you, they can also use the booking form on the page or call 063 304 700 — but offer to do "
        "it for them first.\n"
